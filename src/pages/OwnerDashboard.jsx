...
import { LogOut, LayoutGrid, Home, Calendar, Settings, Plus, X, Clock, DollarSign, CheckCircle, Menu, ArrowLeft } from "lucide-react";
      ...
      <div className="lg:ml-64 p-4 sm:p-6 lg:p-8">
        ...
        {activeTab === "bookings" && (
          <div className="bg-white rounded-xl shadow-md p-6 sm:p-8">
            ...
            <div className="space-y-4">
              {properties.length > 0 && properties.some(p => p.bookings > 0) ? (
                <div className="grid grid-cols-1 gap-4">
                  {properties.filter(p => p.bookings > 0).map(property => (
                    <div key={property.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">{property.bookings} active booking(s)</p>
                          <p className="text-xs text-gray-500 mt-1">📍 {property.district}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-red-600">NPR {(property.price * property.bookings).toLocaleString()}</p>
                          <p className="text-xs text-gray-500 mt-1">Total for bookings</p>
                        </div>
                      </div>
                    </div>
                  ))}
